...
@implementation CATKStorageProvider
...
- (BOOL)storeValueForKey:(nonnull NSString*)key
    ...
    {
        if(error!= NULL){
        *error = [[NSError alloc]initWithDomain:kErrorDomainCATKSecureStorage
                                           code:CATKSSErrorCodeUnknownKey
...
-(nullable id)fetchValueForKey:(nonnull NSString*)key
    ...
    if (!data) {
        if(error!= NULL){
            *error = [[NSError alloc]initWithDomain:kErrorDomainCATKSecureStorage
                                               code:CATKSSErrorCodeNoDataFoundForKey
     ...
     */
        ...
        if(error!= NULL){
            *error = [[NSError alloc]initWithDomain:kErrorDomainCATKSecureStorage
                                               code:CATKSSErrorCodeInvalidDataType
...
-(NSData*)encryptData:(nonnull id<NSCoding>)object
    ...
    if (!object) {
        ...
        {
        *error = [[NSError alloc]initWithDomain:kErrorDomainCATKSecureStorage
                                           code:CATKSSErrorCodeAccessKeyFailure
    ...
    if (!data) {
        if(error!= NULL){
            *error = [[NSError alloc]initWithDomain:kErrorDomainCATKSecureStorage
                                               code:CATKSSErrorCodearchiveFailure
    ...
    NSData *encryptedData=[data AES256EncryptWithKey:secureAccessKey andInitialisationVector:[CATKSKeyGenerator generateInitialisationVectorForKey:secureAccessKey] error:&encryptError];
    if(encryptError)
    {
        if(error!= NULL)
        {
...
-(id)decryptData:(nonnull NSData*)data
    ...
    if (!data) {
        ...
        {
            *error = [[NSError alloc]initWithDomain:kErrorDomainCATKSecureStorage
                                               code:CATKSSErrorCodeInvalidInput
                                           userInfo:[NSDictionary dictionaryWithObject:kErrorDescriptionInvalidParam forKey:NSLocalizedDescriptionKey]];
        }
        return nil;
    ...
    if (!decryptedData) {
        if(error!= NULL){
            *error = [[NSError alloc]initWithDomain:kErrorDomainCATKSecureStorage
                                               code:CATKSSErrorCodeDecryptionError
    ...
    else
    {
        ...
        if (!unarchivedData) {
            if(error!= NULL){
                *error = [[NSError alloc]initWithDomain:kErrorDomainCATKSecureStorage
                                                   code:CATKSSErrorCodeUnarchiveFailure
...
- (void)storeSecureAccessKey:(NSData*)key {
    [CATKSSKeychainService storeValue:key
                             forKey:KEYCHAIN_ACCESS_KEY
                       andTokenType:[CATKSSUtility serviceNameForTokenName:KEYCHAIN_TOKEN_TYPE]];
}
...
- (NSData*)fetchSecureAccessKey {
    NSData *secureAccessKey=[CATKSSKeychainService getValueForTokenType:
                             [CATKSSUtility serviceNameForTokenName:KEYCHAIN_TOKEN_TYPE]
                                                           andKey:KEYCHAIN_ACCESS_KEY error:nil];
    if (secureAccessKey.length<kCCKeySizeAES256) {
        secureAccessKey=[CATKSKeyGenerator generateSecureAccessKey];
        if (secureAccessKey.length<kCCKeySizeAES256) {
            return nil;
        }

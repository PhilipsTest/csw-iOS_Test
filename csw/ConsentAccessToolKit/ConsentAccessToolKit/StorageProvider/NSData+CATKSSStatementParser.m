...
- (NSData *)AES256EncryptWithKey:(NSData *)key
    ...
    {
        if(error!= NULL){
         *error = [[NSError alloc]initWithDomain:kErrorDomainCATKSecureStorage code:CATKSSErrorCodeUnknownKey userInfo:[NSDictionary dictionaryWithObject:kErrorDescriptionEmptyKey forKey:NSLocalizedDescriptionKey]];
        }
        return nil;
    }
    ...
    {
        if(error!= NULL)
        *error = [[NSError alloc]initWithDomain:kErrorDomainCATKSecureStorage code:CATKSSErrorCodeEncryptionError userInfo:[NSDictionary dictionaryWithObject:kErrorDescriptionEncryptionFailure forKey:NSLocalizedDescriptionKey]];
...
- (NSData *)AES256DecryptWithKey:(NSData *)key
    ...
    if(self.length <= kCCBlockSizeAES128 ){
        if(error!= NULL){
            *error = [[NSError alloc]initWithDomain:kErrorDomainCATKSecureStorage code:CATKSSErrorCodeDecryptionError userInfo:[NSDictionary dictionaryWithObject:kErrorDescriptionDecryptionFailure forKey:NSLocalizedDescriptionKey]];
        }
        return nil;
    }else{

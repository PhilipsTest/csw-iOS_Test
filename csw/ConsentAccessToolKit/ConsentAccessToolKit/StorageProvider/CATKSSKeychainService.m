...
static NSString *CATKSSKeychainUtilsErrorDomain = @"com.philips.platform.catk.KeychainUtilsErrorDomain";

@implementation CATKSSKeychainService
...
+ (NSData *)getValueForTokenType:(NSString *)tokenType andKey:(NSString *)key error:(NSError * __autoreleasing *)error {
    ...
    if (!tokenType || !key) {
        if (error != nil) {
            *error = [NSError errorWithDomain: CATKSSKeychainUtilsErrorDomain code: -2000 userInfo: nil];
        }
        return nil;
    }
    ...
    if (status != noErr) {
        ...
        if (error != nil && status != errSecItemNotFound) {
            ...
            *error = [NSError errorWithDomain: CATKSSKeychainUtilsErrorDomain code: status userInfo: nil];
        }
        return nil;
    }
    ...
    if (status != noErr) {
        if (status == errSecItemNotFound) {
            if (error != nil) {
                *error = [NSError errorWithDomain: CATKSSKeychainUtilsErrorDomain code: -1999 userInfo: nil];
            }
        }
        else {
            if (error != nil) {

...
-(void)downloadConfigFromCloud:(void(^)(AIACRefreshResult, NSError *))completionHandler {
    ...
    NSString * cloudServiceID = [self.aiAppInfra.appConfig getPropertyForKey:@"appconfig.cloudServiceId"
                                                                       ...
                                                                       error:&configError];
    if (cloudServiceID) {
        [self.aiAppInfra.serviceDiscovery getServiceUrlWithCountryPreference:cloudServiceID
                                                       withCompletionHandler:^(NSString *serviceURL, NSError *serviceURLError) {
                                                           if (serviceURL) {
                                                               NSString * savedURL = [self.aiAppInfra.storageProvider fetchValueForKey:cloudURLKey
                                                                                                                                 error:nil];
                                                               if (![serviceURL isEqualToString:savedURL]) {
                                                                   [self clearCloudConfigFile:nil];
                                                                   [self.aiAppInfra.RESTClient GET:serviceURL
                                                                                        parameters:nil
                                                                                          progress:nil

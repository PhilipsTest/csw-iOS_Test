                    ...
                    completion:(void(^)(NSString*serviceURL,NSError *error))completionHandler
{
    if (preference == AIRESTServiceIDPreferenceCountry) {
        [self.appInfra.serviceDiscovery getServiceUrlWithCountryPreference:serviceID withCompletionHandler:^(NSString *serviceURL, NSError *countryPrefError) {
            [self createCompleteURL:serviceURL pathComponent:pathComponent error:countryPrefError completion:^(NSString *URL, NSError *countryFinalerror) {
        ...
        [self.appInfra.serviceDiscovery getServiceUrlWithLanguagePreference:serviceID withCompletionHandler:^(NSString *serviceURL, NSError *langPrefError) {
            [self createCompleteURL:serviceURL pathComponent:pathComponent error:langPrefError completion:^(NSString *URL, NSError *langFinalerror) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    completionHandler(URL, langFinalerror);
                });
            }];
        }];
    }

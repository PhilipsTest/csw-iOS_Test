...
@objc public class AIClouldLoggingSyncManager: NSObject,AIConsentStatusChangeProtocol {
    ...
    func consentValueCheck(completion:@escaping CompletionHandler){
        ...
        if let consentManager = self.appinfra?.consentManager, let cloudConsentIdentifier = self.appinfra?.logging.getCloudLoggingConsentIdentifier?(), let cloudConsentDefinition = self.getCloudLoggingConsentDefinition(consentType: cloudConsentIdentifier){

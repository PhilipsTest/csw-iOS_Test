 ...
 * @param completionHandler is a block which asynchronously returns in main queue using onSuccess the URL of the requested service or returns onError the error code when retrieval failed.
 * @since 1.0.0
 */
 ...
 * Returns the URL for a specific service with a preference for the current language.
 * @param serviceId name of the service for which the URL is to be retrieved
 * @param completionHandler asynchronously returns in main queue using onSuccess the URL of the requested service or returns onError the error code when retrieval failed.
...
/**
 * Returns the URL for a specific service with a preference for the current home country.we first select the URL set with the best matching locale before looking for the requested service ID in this set.
 * @param serviceId name of the service for which the URL is to be retrieved
 * @param completionHandler is a block which asynchronously returns in main queue using onSuccess the URL of the requested service or returns onError the error code when retrieval failed.
 * @since 1.0.0
 */
...
/**
 * Returns the URL and locale for a specific services with a preference for the current language.
 * @param serviceIds array of service name for which the URL and locale is to be retrieved
...
/**
 * Returns the service url and locale for a specific services with a preference for the current home country.we first select the URLs set with the best matching locale before looking for the requested service ID in this set.
 * @param serviceIds array of service names for which the URLs and locale is to be retrieved
 * @param completionHandler A block object to be executed when it fetches urls successfully. This block has no return value and has two arguments: the services object, and the error object. The services object is a dictionary with service id as key and AISDService object will be value. It will contain the service discovery urls and key map values(if key indexes are available for that service id, see key manager module for more information)
 * @since 1.0.0
 */
 ...
 * @param serviceId name of the service for which the URL is to be retrieved
 * @param completionHandler is a block which asynchronously returns in main queue using onSuccess the recommended locale for the requested service or returns onError the error code when retrieval failed.
 * @since 1.0.0
 */

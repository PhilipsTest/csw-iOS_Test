    ...
    dispatch_async(queue, ^{
        ...
        [self.sdManager homeCountryCodeWithCompletion:^(NSString *homeCountryCode, NSString *sourceType, NSError *error) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if (error) [AIInternalTaggingUtility tagError:error  errorCategory:kSDEvent errorMessageFormat:kAilTagHomeCountryError];
...
-(void)setHomeCountry:(NSString *)countryCode
{
    if (![countryCode isEqualToString:[self.sdManager savedCountryCode]]) {
        if ([self validateCountryCode:countryCode]){
            ...
            [AIInternalTaggingUtility tagSuccess:kSDEvent infoMessageFormat:@"%@ - %@",kAilTagSetHomeCountry,countryCode];
            ...
            [AIInternalTaggingUtility tagSuccess:kSDEvent infoMessageFormat:@"%@ - %@",kAilTagSetHomeCountryInvalid,countryCode];
        }
    }
}

-(void)getServiceUrlWithLanguagePreference:(NSString *)serviceId
                     withCompletionHandler:(void(^)(NSString*serviceURL,NSError *error))completionHandler
{
    [self loadServiceDiscoveryWithcompletionHandler:^(AISDURLs*  SDURLs, NSError *error) {
        if (SDURLs) {
            NSString * URL = [SDURLs getServiceURLWithServiceID:serviceId Preference:AISDLanguagePreference];
            if (URL) {
                completionHandler(URL, error);
            }
            else {
                completionHandler(URL, [AISDManager getSDError:AISDCannotFindURLError]);
            }
        }
        else {
...
{
    [self getServiceUrlWithLanguagePreference:serviceId withCompletionHandler:^(NSString * _Nonnull serviceURL, NSError * _Nonnull error) {
        if (serviceURL) {
            NSString * URLString = [self applyURLParameters:serviceURL parameters:replacement];
            if (URLString) {
                completionHandler(URLString, error);
            }
            else {
                completionHandler(nil, [AISDManager getSDError:AISDMalformedURLError]);
            }
        }
        else {
            completionHandler(nil, error);
...
-(void)getServiceUrlWithCountryPreference:(NSString *)serviceId
                    withCompletionHandler:(void(^)(NSString*serviceURL,NSError *error))completionHandler
{
    [self loadServiceDiscoveryWithcompletionHandler:^(AISDURLs*  SDURLs, NSError *error) {
        if (SDURLs) {
            NSString * URL = [SDURLs getServiceURLWithServiceID:serviceId Preference:AISDCountryPreference];
            if (URL) {
                completionHandler(URL, error);
            }
            else {
...
-(void)getServiceUrlWithCountryPreference:(NSString *)serviceId
                    withCompletionHandler:(void(^)(NSString*serviceURL,NSError *error))completionHandler
                              replacement:(NSDictionary *)replacement
{
    [self getServiceUrlWithCountryPreference:serviceId withCompletionHandler:^(NSString * _Nonnull serviceURL, NSError * _Nonnull error) {
        if (serviceURL) {
            NSString * URLString = [self applyURLParameters:serviceURL parameters:replacement];
            if (URLString) {
                completionHandler(URLString, error);
...
-(void)getServiceLocaleWithLanguagePreference:(NSString *)serviceId
                        withCompletionHandler:(void(^)(NSString*serviceLocale,NSError *error))completionHandler
{
    [self loadServiceDiscoveryWithcompletionHandler:^(AISDURLs*  SDURLs, NSError *error) {
        if (SDURLs) {
            NSString * locale = [SDURLs getLocaleWithPreference:AISDLanguagePreference];
            if (locale) {
                completionHandler(locale, error);
            }
            else {
                completionHandler(locale, [AISDManager getSDError:AISDCannotFindLocaleError]);
            }
        }
        else {
            completionHandler(nil, error);
...
-(void)getServiceLocaleWithCountryPreference:(NSString *)serviceId
                       withCompletionHandler:(void(^)(NSString*serviceLocale,NSError *error))completionHandler
{
    [self loadServiceDiscoveryWithcompletionHandler:^(AISDURLs*  SDURLs, NSError *error) {
        if (SDURLs) {
            NSString * locale = [SDURLs  getLocaleWithPreference:AISDCountryPreference];
            if (locale) {
                completionHandler(locale, error);
            }
            else {
...
{
    [AIInternalTaggingUtility tagSuccess:kSDEvent infoMessageFormat:kAilTagSDRefresh];
    [self fetchServiceDicoveryWithcompletionHandler:^(AISDURLs *SDURLs, NSError *error) {
        if(error){ [AIInternalTaggingUtility tagError:error errorCategory:kSDEvent errorMessageFormat:kAilTagSDRefreshError];}
...
-(NSString *)applyURLParameters:(NSString *)URLString parameters:(NSDictionary *)map {
    ...
    if (!URLString) {
        ...
        [AIInternalTaggingUtility tagError:nil errorCategory:kSDEvent errorMessageFormat:kAilTagMalformedUrl];
    }
    return URLString;
}
...
-(void)loadServiceDiscoveryWithcompletionHandler:(void (^)( AISDURLs*  SDURLs, NSError *  error))completionHandler {
    ...
    dispatch_async(queue, ^{
        ...
        if (self.cachedSDURLs == nil) {
            ...
            if (self.cachedSDURLs) {
                [AIInternalTaggingUtility tagSuccess:kSDEvent infoMessageFormat:kAilTagSDDatafromCache];
            }
        }
        ...
        if (refreshRequired || self.cachedSDURLs == nil || urlsOlderThan24Hours) {
            ...
            if (refreshRequired) {
                [AIInternalTaggingUtility tagSuccess:@"AISDManager" infoMessageFormat:kAilTagSDShouldRefresh];
                [self.sdManager clearDownloadedURLs];
            }

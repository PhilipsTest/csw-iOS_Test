...
- (void)downloadPlatformData:(void (^)(AISDURLs *, NSError *))completionHandler SDURLs:(AISDURLs *)SDURLs {
    ...
    if (platformURL) {
        ...
        [AIInternalTaggingUtility tagSuccess:@"AISDManager" infoMessageFormat:@"%@ - %@",kAilTagDownloadPlatform,platformURL];
...
- (void)downloadPlatformAndPropositionData:(void (^)(AISDURLs *, NSError *))completionHandler {
    ...
    if (propositionURL) {
        ...
        [AIInternalTaggingUtility tagSuccess:@"AISDManager"
                           infoMessageFormat:@"%@ - %@",kAilTagDownloadProposition,propositionURL];
                                   ...
                                   completionHandler:^(NSDictionary*  responseProposition, NSError *propositionError) {
                                       if (responseProposition) {
                                           ...
                                           if (countryCode == nil) {
                                               ...
                                               [AIInternalTaggingUtility tagSuccess:@"AISDManager"
                                                                  infoMessageFormat:@"%@ - %@",kAilTagCountryFromIP,sdModel.country];
...
-(void)homeCountryCodeWithCompletion:(void(^)(NSString *countryCode, NSString *sourceType, NSError *error))completionHandler {
    ...
    if ([countryCode length] <= 0 ) {
        ...
        if (countryCode) {
            [AIInternalTaggingUtility tagSuccess:@"AISDManager"
                               infoMessageFormat:@"%@ - %@",kAilTagCountryFromSim,countryCode];
...
-(void)cacheDownloadedURLs:(NSDictionary *)downloadedJson
    ...
    if (error) {
        ...
        if (success) {
            ...
            [AIInternalTaggingUtility tagError:writeError
                                 errorCategory:@"AISDManager"
                            errorMessageFormat:kAILTagSDDataStoreFailed];
        }
...
-(void)clearDownloadedURLs {
    [AIInternalTaggingUtility tagSuccess:@"AISDManager" infoMessageFormat:kAilTagClearSDData];
...
-(void)saveCountryCode:(NSString *)countryCode sourceType:(NSString *)sourceType {
    ...
    if(error) {
        [AIInternalTaggingUtility tagError:error
                             errorCategory:@"AISDManager"
                        errorMessageFormat:kAilTagSetHomeCountrySaveError];
    }
